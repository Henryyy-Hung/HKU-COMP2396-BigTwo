...
public class BigTwoGUI implements CardGameUI
{
	...
	public BigTwoGUI(BigTwo game)
	{
		...
		this.connectItem.addActionListener(new ConnectMenuItemLisner());
	...
	// a menu item for player to connect the server
	...
	class BigTwoPanel extends JPanel
	{
		...
		public BigTwoPanel()
		{
			...
			for (int i = 0; i < playerList.size(); i++)
			{
				...
				PlayerInfo infoPanel = new PlayerInfo(player, playerID);
	...
	class PlayerInfo extends JPanel
	{
		...
		PlayerInfo(CardGamePlayer player, int playerID)
		{
			this.setOpaque(false);
			this.setSize(INFO_PANEL_DIMENSION);
			this.setBackground(Color.black);
			this.setLayout(new GridBagLayout());
			this.add(new PlayerName(player,playerID), new GridBagConstraints(0,0,1,1,1.0,0.1,GridBagConstraints.CENTER,GridBagConstraints.BOTH,new Insets(0, 10, 0, 10),0,0));
	...
	class PlayerName extends JLabel
	{
		...
		PlayerName(CardGamePlayer player, int playerID)
		{
			this.player = player;
		...
		public void paintComponent(Graphics g)
		{
			...
			if (playerID == activePlayer)
				...
				this.setForeground(new Color(112, 218, 248));
			else
	...
	class CardDisplayPanel extends JLayeredPane implements MouseListener
	{
		...
		public void paintComponent(Graphics g)
		{
			...
			if (playerID == clientPlayer && activePlayer == clientPlayer)
	...
	class ChatInputListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				String msg =  input + "\n";
				game.getBigTwoClient().sendMessage(new CardGameMessage(CardGameMessage.MSG, -1, msg));
				chatInput.setText("");
			}
		}
	}
	
	class ConnectMenuItemLisner implements ActionListener
	{
		@Override
		public void actionPerformed(ActionEvent e) 
		{
			if (game.getBigTwoClient().getIsConnected() == false)
			{
				game.getBigTwoClient().connect();
			}
			else
			{
				printDialog("The Client has already connected !!!");
			}
	...
	/**
	 * Sets the index of the client player.
	 * 
	 * @param clientPlayer the index of the client player 
	 */
	public void setClientPlayer(int clientPlayer)
	{
		if (clientPlayer < 0 || clientPlayer >= playerList.size())
		{
			this.clientPlayer = -1;
		}
		else
	...
	/**
	 * Prints the specified string to the GUI chat area.
	 * 
	 * @param chat the string to be printed to the GUI
	 */
