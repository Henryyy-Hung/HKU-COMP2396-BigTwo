...
public class BigTwo implements CardGame
{
	...
	// the idx of this player
	private int clientPlayerIdx; 
	
	// a BigTwoUI object for providing the user interface.
	private BigTwoGUI gui;
	
	// a BigTwo client for connecting server
	private BigTwoClient client;
	
	public void setBigTwoClient(BigTwoClient client)
	{
		this.client = client;
	}
	
	public void setClientPlayerIdx(int idx)
	{
		this.clientPlayerIdx = idx;
	}
	...
	public void start(Deck deck)
	{
		...
		if (this.clientPlayerIdx == this.currentPlayerIdx)
			this.gui.enable();
		else
			this.gui.disable();
	...
	public void checkMove(int playerIdx, int[] cardIdx)
	{
		...
		else
		{
			...
			{		
				...
				String gameResult = "";
				gameResult = gameResult + ("Game ends\n");
				for (int i = 0; i < this.getNumOfPlayers(); i++)
				{
					String name = this.getPlayerList().get(i).getName();
					int remainingCards = this.getPlayerList().get(i).getCardsInHand().size();
					if ( remainingCards > 0)
						gameResult = gameResult + ( (name+" has "+Integer.toString(remainingCards)+" cards in hand." + "\n") );
					else
						gameResult = gameResult + ( (name+" wins the game." + "\n") );
				}
				this.gui.printDialog(gameResult);
